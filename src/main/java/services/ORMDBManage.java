...
//adapted from github user: vacax 03/06/2016
...
public class ORMDBManage<T> {

    private static EntityManagerFactory emf;
    private Class<T>  entityClass;

    public ORMDBManage(Class<T> entityClass){
        if (emf == null){
    ...
    public T modify (T entity) throws PersistenceException{
        EntityManager em = getEntityManager();
        em.getTransaction().begin();
        try {
            em.merge(entity);
    ...
    public boolean delete (Object entityId) throws PersistenceException{
        ...
        em.getTransaction().begin();
        try {
            T entity = em.find(this.entityClass, entityId);
            em.remove(entity);
            em.getTransaction().commit();
            ok = true;
        }finally {
